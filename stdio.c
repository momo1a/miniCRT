...
/*不需要做任何初始化   这个函数形同虚设  真正的IO初始化时要做很多操作的*/
int mini_crt_io_init()
{
	return 1;
}

#ifdef WIN32  // 实现windows文件操作函数
#include <windows.h>
...
/*读文件*/
int fread(const void* buffer,int size,int count,FILE *stream)
{
	int read = 0;
...
FILE *fopen(const char* filename,const char* mode)
{
	int fd = -1;
	int flags = 0;
	int access = 00700;  // 创建文件时的权限
	// 来自 /usr/include/bits/fcntl.h
#define O_RDONLY 00
#define O_WRONLY 01
#define O_RDWR   02
#define O_CREAT  0100
